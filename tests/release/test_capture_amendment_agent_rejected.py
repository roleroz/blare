"""live: capture the amendment-agent-rejected fixture against the real
...
prompts an agent-proposed amendment, rejected at its re-presentation (the
proposed repairs are restored). Tagged `live`, `exclusive` -- see
...
def test_live_capture_amendment_agent_rejected(tmp_path: Path) -> None:
    """A chat-prompted agent-proposed amendment, rejected at its
    ...
    cap = capture.capture_amendment_agent(tmp_path, "amendment-agent-rejected", approve=False)
    finalize_capture(cap.record_dir, capture.FIXTURES_ROOT / "amendment-agent-rejected")

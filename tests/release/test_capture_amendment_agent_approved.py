"""live: capture the amendment-agent-approved fixture against the real
`~/external_git/miniflux_v2` checkout with the live Claude Agent SDK (T4.1's
continuation).

Runs `blare analyze` (a real re-analysis, R16, since `.blare/` already
exists); chat at phase 4's checkpoint prompts an agent-proposed amendment,
approved at its re-presentation. Tagged `live`, `exclusive` -- see

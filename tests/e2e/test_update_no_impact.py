...
def test_e2e_update_no_impact_confirmed_changes_only_the_sha(tmp_path: Path) -> None:
    """A docs-only delta: the agent concludes no_impact with an empty queue, the
    no-impact screen is presented, and approving it advances only the recorded
    SHA -- every canonical entry file keeps its exact bytes."""
    ...
    assert "no code paths affected" in output
